...
class DistTensorParallelExampleTest(DTensorTestBase):
    ...
    def test_loss_parallel_log_softmax_non_cross_entropy(self):
        ...
        Covers 2D/3D inputs, negative dim, and fp16 (half_to_float) dtype."""
        device_mesh = self.build_device_mesh()
        comm_mode = CommDebugMode()
        channel_size = 16
        ...
        test_cases = [
            ...
            ((8, channel_size), 1, torch.float32),
            ((8, channel_size), -1, torch.float32),
            ((4, channel_size, 6), 1, torch.float32),
            ((8, channel_size), 1, torch.float16),
        ]
        for input_size, softmax_dim, dtype in test_cases:
            ...
            (
                F.log_softmax(x_ref, dim=softmax_dim, dtype=ref_dtype) * c
            ).sum().backward()
            ...
            with loss_parallel():
                ...
                self.assertEqual(comm_mode.get_total_counts(), 3)
                self.assertEqual(
                    comm_mode.get_comm_counts()[c10d_functional.all_reduce], 3
                )
